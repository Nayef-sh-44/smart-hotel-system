...
# 1. Imports
code = code.replace("import {\n  Star,", "import {\n  Banknote, Utensils, ShoppingBag, HeartPulse, Train, TreePine, Ticket, Church, Loader2, Navigation, Compass, MapPin as MapLucide,\n  Star,")

# 2. Add PlaceIcon and getPlaceScore right after component definition
cmp_def = "export default function HotelDetail() {"
if cmp_def not in code:
    print("Component def not found!")
    exit(1)
...
code = code.replace(cmp_def, cmp_def + "\n" + blocks)

# 3. Update fetchNearbyPlaces
...
ui_regex = r"\{\/\*\s*Nearby Services\s*\*\/\}.*?No landmarks listed\.<\/p>\s*\)\}\s*<\/div>"
new_ui = """
...
# 5. Fix Map filter UI mapping
map_filter_regex = r"\{\s*nearbyPlaces\.filter\(p => mapFilter === 'all' \|\| p\.group === mapFilter\)\.map\(\(place\) => \{"
new_map_filter = """
{nearbyPlaces.filter(p => mapFilter === 'all' || p.category === mapFilter || (mapFilter === 'attraction' && p.category === 'family_attraction'))
  ...
  .map((place) => {
"""
code = re.sub(map_filter_regex, new_map_filter.strip(), code)
