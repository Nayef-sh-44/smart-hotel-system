...
// 2. Update getPlaceScore to use exact categories and exact scoring requested by the user
const scoreRegex = /const getPlaceScore = \(place\) => \{[\s\S]*?return score;\s*\};/;
const newScore = `
...
`;
code = code.replace(scoreRegex, newScore.trim());

// 3. Update PlaceIcon helper to check place.displayCategory (which contains the old category string)
const iconHelperRegex = /const PlaceIcon = \(\{ category, className, color \}\) => \{[\s\S]*?return <MapPin \{\.\.\.props\} \/>;\s*\};/;
const newIconHelper = `
...
`;
code = code.replace(iconHelperRegex, newIconHelper.trim());

// 4. Update the render logic to use displayCategory and not filter by tripType group!
const uiBlockRegex = /\{\/\* Real Nearby Services & Attractions \*\/\}[\s\S]*?<\/div>\s*<\/div>/;
...
`;
code = code.replace(uiBlockRegex, newUIBlock.trim());

// 5. Update Map UI to use displayCategory instead of category
const mapPopupRegex = /<span className="px-2 py-0\.5 rounded text-white \\nfont-semibold text-\[10px\]" style={{backgroundColor: place\.color}}>{place\.category}<\/span>/;
code = code.replace(mapPopupRegex, '<span className="px-2 py-0.5 rounded text-white \nfont-semibold text-[10px]" style={{backgroundColor: place.color}}>{place.displayCategory}</span>');
