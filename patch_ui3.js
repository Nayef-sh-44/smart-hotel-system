...
// 3. Remove fetchNearbyPlaces from fetchHotelDetails
c = c.replace(
  `if (res.data.latitude && res.data.longitude) {\n          fetchNearbyPlaces(res.data.latitude, res.data.longitude);\n        }`,
  `// handled by useEffect`
);

// 4. Update UI: Replace the map filter UI
const uiStart = c.indexOf('<div className="flex flex-wrap gap-2">');
const uiEndStr = '              <div style={{ height: \'500px\'';
const uiEnd = c.indexOf(uiEndStr);

const newUi = `<div className="flex flex-col gap-4 mb-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden relative">
`;
c = c.substring(0, uiStart) + newUi + c.substring(uiEnd);

// 5. Update Map Marker logic
...
// 6. Add list after map
const mapEnd = c.indexOf('</MapContainer>');
const mapEndCloseDiv = c.indexOf('</div>', mapEnd);

const newList = `</MapContainer>
                </div>
...
c = c.substring(0, mapEnd) + newList + c.substring(mapEndCloseDiv + 6); // skip </div>
