...
if "calculatePricing" not in content:
    ...
    content = content[:end_import+1] + "import { calculatePricing } from '../services/pricingService.js';\n" + content[end_import+1:]

old_func_start = content.find("export const getCompetitorBenchmarking")
old_func_end = content.find("export const", old_func_start + 1)
if old_func_end == -1: old_func_end = len(content)
...
new_func = """export const getCompetitorBenchmarking = async (req, res, next) => {
  try {
    ...
    if (priceDiffPercent < -5) {
      priceInsights = `Your average room price is ${Math.abs(priceDiffPercent)}% below the competitor average for this period. Consider raising prices to capture more revenue if occupancy allows.`;
    } else if (priceDiffPercent > 5) {
      priceInsights = `Your average room price is ${priceDiffPercent}% above the competitor average for this period. Ensure your amenities justify the premium.`;
    } else {
    ...
    let occInsights = '';
    if (occupancyDiff < -5) {
      occInsights = `Your occupancy is ${Math.abs(occupancyDiff)} percentage points below the competitor average for this period. Consider running a Flash Deal.`;
    } else if (occupancyDiff > 5) {
      occInsights = `Your occupancy is ${occupancyDiff} percentage points above the competitor average for this period! Great job capturing demand.`;
    } else {
