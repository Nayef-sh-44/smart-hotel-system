...
const iconHelperStart = 'const PlaceIcon = ({ category, className, color }) => {';
const iconHelperEnd = 'return <MapPin {...props} />;\n  };';
const iconHelperStartIndex = code.indexOf(iconHelperStart);
const iconHelperEndIndex = code.indexOf(iconHelperEnd) + iconHelperEnd.length;
...
`;
code = code.substring(0, iconHelperStartIndex) + newIconHelper.trim() + code.substring(iconHelperEndIndex);


// 4. Update the render logic to use displayCategory and not filter by tripType group!
const uiBlockStart = '{/* Real Nearby Services & Attractions */}';
const uiBlockEnd = 'No nearby services found within 3km."}\n                  </p>\n                </div>\n              )}\n            </div>';
